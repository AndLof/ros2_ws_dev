...
#include "nav2_pure_controller/pure_controller.hpp"
#include "nav2_core/exceptions.hpp"
#include "nav2_util/node_utils.hpp"
#include "nav2_util/geometry_utils.hpp"
#include "nav2_costmap_2d/costmap_filters/filter_values.hpp"

//#include "custom_msg_srv/msg/trajectory.hpp"
...
namespace nav2_pure_controller
{

void PureController::configure(
  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
{
  ...
  RCLCPP_ERROR(logger_, "AAAAAAAAAAAAAAAAAAAAAA");
  ...
  //pub_ = node->create_publisher<custom_msg_srv::msg::Trajectory>("goaltospot", 1);
  pub_ = node->create_publisher<geometry_msgs::msg::PoseStamped>("goaltospot", 1);
  pub_goal = node->create_publisher<std_msgs::msg::String>("goalreached", 1);
...
void PureController::cleanup()
{
  RCLCPP_INFO(
    logger_,
    "Cleaning up controller: %s of type"
    "pure_controller::PureController",
    plugin_name_.c_str());
...
void PureController::activate()
{
  ...
  pub_->on_activate();
  pub_goal->on_activate();

  // Add callback for dynamic parameters
  auto node = node_.lock();
  dyn_params_handler_ = node->add_on_set_parameters_callback(
    std::bind(
      &PureController::dynamicParametersCallback,
      this, std::placeholders::_1));
}

void PureController::deactivate()
{
  RCLCPP_INFO(
    logger_,
...
std::unique_ptr<geometry_msgs::msg::PointStamped> PureController::createCarrotMsg(
...
double PureController::getLookAheadDistance(
...
geometry_msgs::msg::TwistStamped PureController::computeVelocityCommands(
  const geometry_msgs::msg::PoseStamped & pose,
  const geometry_msgs::msg::Twist & speed,
  nav2_core::GoalChecker * goal_checker)
{
  RCLCPP_ERROR(logger_, "VELOCITIESSSSSSSSSSSSSSSSSSSSSS");
  ...
  if (use_collision_detection_ && isCollisionImminent(pose, linear_vel, angular_vel, carrot_dist)) {
    throw nav2_core::PlannerException("PureController detected collision ahead!");
  }
...
//  return cmd_vel;
  ...
  /////////////////////////////////////////////////////////////////
  //custom_msg_srv::msg::Trajectory goal_pub;
  geometry_msgs::msg::PoseStamped goal_pub;
  geometry_msgs::msg::PoseStamped global_pose;
  costmap_ros_->getRobotPose(global_pose);

  robot_curr_pose.x = global_pose.pose.position.x;
  robot_curr_pose.y = global_pose.pose.position.y;
  robot_curr_orien = getEulerAngles(global_pose)[2];
  std::cout << "CURRENT POSITION " << robot_curr_pose.x << " " << robot_curr_pose.y << " " << robot_curr_orien << std::endl;
  ...
  for (i = 0; i < ((int)global_plan_.poses.size()); ++i)
  {
    std::cout << "POSE " << i << " : " << global_plan_.poses[i].pose.position.x << ", " << global_plan_.poses[i].pose.position.y << std::endl;
    float dist = hypot(global_plan_.poses[i].pose.position.x - robot_curr_pose.x, global_plan_.poses[i].pose.position.y - robot_curr_pose.y);
    if (dist < min_dist)
    {
      index_closest = i;
      min_dist = dist;
      std::cout << index_closest << " " << min_dist << std::endl;
    }
  }

  next_index = index_closest + 25;
  next_index = std::min(static_cast<int>(global_plan_.poses.size()) - 1, next_index);

  float temp_target_x = global_plan_.poses[next_index].pose.position.x;
  float temp_target_y = global_plan_.poses[next_index].pose.position.y;
  float temp_orien_x = global_plan_.poses[next_index].pose.orientation.x;
  float temp_orien_y = global_plan_.poses[next_index].pose.orientation.y;
  float temp_orien_z = global_plan_.poses[next_index].pose.orientation.z;
  float temp_orien_w = global_plan_.poses[next_index].pose.orientation.w;

  std::cout << "TARGET X: " << temp_target_x << " Y: " << temp_target_y << " OR X: " << temp_orien_x << " OR Y: " << temp_orien_y << " OR Z: " << temp_orien_z << std::endl;
  /*
  goal_pub.target_pose.pose.position.x = temp_target_x;
  goal_pub.target_pose.pose.position.y = temp_target_y;
  goal_pub.target_pose.pose.orientation.x = temp_orien_x;
  goal_pub.target_pose.pose.orientation.y = temp_orien_y;
  goal_pub.target_pose.pose.orientation.z = temp_orien_z;
  goal_pub.target_pose.pose.orientation.w = temp_orien_w;
  */
  goal_pub.header.stamp = rclcpp::Clock().now(); //node_->now();  // oppure rclcpp::Clock().now() se non hai node_->now()
  goal_pub.header.frame_id = global_plan_.header.frame_id;

  goal_pub.pose.position.x = temp_target_x;
  goal_pub.pose.position.y = temp_target_y;
  goal_pub.pose.position.z = 0.0;  // opzionale se vuoi esplicitare lo Z

  goal_pub.pose.orientation.x = temp_orien_x;
  goal_pub.pose.orientation.y = temp_orien_y;
  goal_pub.pose.orientation.z = temp_orien_z;
  goal_pub.pose.orientation.w = temp_orien_w;
  pub_->publish(goal_pub);
//  sleep(5);
  std::cout << "60 secs pass" << std::endl;
...
std::vector<double> PureController::getEulerAngles(geometry_msgs::msg::PoseStamped& pose)
{
   // RCLCPP_ERROR(logger_, "VELOCITIESSSSSSSSSSSSSSSSSSSSSSSS");
...
bool PureController::shouldRotateToPath(
...
bool PureController::shouldRotateToGoalHeading(
  const geometry_msgs::msg::PoseStamped & carrot_pose)
{
...
void PureController::rotateToHeading(
...
geometry_msgs::msg::Point PureController::circleSegmentIntersection(
...
geometry_msgs::msg::PoseStamped PureController::getLookAheadPoint(
...
bool PureController::isCollisionImminent(
...
bool PureController::inCollision(
  const double & x,
  const double & y,
  const double & theta)
{
...
double PureController::costAtPose(const double & x, const double & y)
{
  unsigned int mx, my;
  ...
  if (!costmap_->worldToMap(x, y, mx, my)) {
    ...
    throw nav2_core::PlannerException(
            "PureController: Dimensions of the costmap are too small "
...
double PureController::approachVelocityScalingFactor(
...
void PureController::applyApproachVelocityScaling(
...
void PureController::applyConstraints(
...
void PureController::setPlan(const nav_msgs::msg::Path & path)
{
  global_plan_ = path;
}

void PureController::setSpeedLimit(
...
nav_msgs::msg::Path PureController::transformGlobalPlan(
  const geometry_msgs::msg::PoseStamped & pose)
{
...
double PureController::findVelocitySignChange(
  const nav_msgs::msg::Path & transformed_plan)
{
...
bool PureController::transformPose(
...
double PureController::getCostmapMaxExtent() const
{
...
PLUGINLIB_EXPORT_CLASS(
  nav2_pure_controller::PureController,
